...
// Initialize the SDK with the Beyond PROD API endpoint
try {
  beyond.initialize({
    apiUrl: "https://api.beyondnetwork.xyz", // Production API endpoint
    debug: false, // Disable debug in production
    storagePrefix: 'beyond_e2e_prod_'
  });
...
async function runE2ETest() {
  try {
    console.log('\n=== Beyond AI SDK E2E Test (PRODUCTION Environment) ===\n');
    ...
    try {
      ...
      // Calculate and display remaining credits
      const monthlyLimit = parseFloat(creditBalance.monthlyLimit);
      const monthlyUsage = parseFloat(creditBalance.monthlyCurrentUsage);
      const remainingCredits = monthlyLimit - monthlyUsage;
      console.log(`      - Remaining Monthly Credits: ${remainingCredits.toFixed(8)}`);
      
      // Validate data types and structure
      const requiredFields = ['monthlyLimit', 'monthlyCurrentUsage', 'totalCreditsUsed', 'totalCreditsPurchased'];
      const missingFields = requiredFields.filter(field => !creditBalance.hasOwnProperty(field));
      
      if (missingFields.length === 0) {
        console.log('   ✅ Credit balance structure validated');
      } else {
        console.log(`   ❌ Missing fields in credit balance: ${missingFields.join(', ')}`);
      }
      
      // Validate that all values are numeric strings
      const allNumeric = requiredFields.every(field => {
        const value = creditBalance[field];
    ...
    console.log(`   Has tokens: ${hasTokens ? beyond.storage.getAccessToken().substring(0, 100) + '...' : 'false'}`);
...
// Start the test
console.log('🚀 Starting Beyond AI SDK E2E Test (Production)...\n');
runE2ETest(); 